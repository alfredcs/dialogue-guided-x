...
from mm_extract_files import do_faiss_query, update_faiss_store
...
with st.sidebar:
    ...
    option = st.selectbox('Choose Model',('anthropic.claude-v2:1', 'mistral-7b', 'gemini-pro', 'gemini-pro-vision', 'gpt-4-1106-preview', 'gpt-4-vision-preview', 'stability.stable-diffusion-xl-v1:0', 'amazon.titan-image-generator-v1', 'llava-v1.5-13b-vision', 'dall-e-3', 'gpt-3.5-turbo-0125'))
    ...
    if doc_urls:
        ...
        st.session_state.messages.append({"role": "assistant", "content": msg})
        st.chat_message("ai").write(msg)
    if upload_docs or doc_urls:
        update_faiss_store(",".join(upload_doc_names), doc_urls)
...
if upload_images or image_url:
    if option != "gemini-pro-vision" and option != "gpt-4-vision-preview" and option != 'llava-v1.5-13b-vision' and option != 'stability.stable-diffusion-xl-v1:0' and option != 'amazon.titan-image-generator-v1':
